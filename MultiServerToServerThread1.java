...
public class MultiServerToServerThread1 extends MultiServer1 implements  Runnable {
    ...
    MultiServerToServerThread1(Socket cSocket){
            ...
            switch (action) {
                ...
                case 'i':
                    ...
                    if (cId1 == server3){
                        ...
                        recursive_locking_and_update(it, forwarding, request_parts, 1);
    ...
    private void recursive_locking_and_update(Iterator<Entry<Integer, Object>> it, int forwarding, String[] request_parts, int next_server_to_send){
        Thread.currentThread().setPriority(10);
        if (it.hasNext()) {
            synchronized (account_lock.get(it.next().getKey())){
                System.out.println("I am here, in recursion!");
                recursive_locking_and_update(it, forwarding, request_parts, next_server_to_send);
            }
        }
        else{
            ...
            try{
                ...
                if (forwarding == 2){
                    try {
                        cSocketSS = new Socket("localhost", servers_ports[next_server_to_send]);
                        sockWriterSS = new PrintWriter(cSocketSS.getOutputStream());
